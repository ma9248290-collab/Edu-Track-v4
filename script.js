...
// ==========================================
const TEACHER_NAME = "شيفو"; // تقدر تغير الاسم ده لأي اسم مدرس لما تبيع السيستم
...
// 1. دالة فتح نافذة الإغلاق (ذكية: تخفي الامتحان والواجب لو مش موجودين)
function toggleSessionStatus(id) { 
    ...
    if(session.status === 'open') { 
        ...
        // فحص هل يوجد امتحان أو واجب مطابق لهذه الحصة بالتحديد؟
        const hasExam = exams.some(e => e.group === session.group && e.date === session.date);
        const hasHw = homeworks.some(h => h.group === session.group && h.date === session.date);
        
        document.getElementById('sendExamCheck').parentElement.style.display = hasExam ? 'flex' : 'none';
        document.getElementById('sendExamCheck').checked = hasExam;
        
        document.getElementById('sendHwCheck').parentElement.style.display = hasHw ? 'flex' : 'none';
        document.getElementById('sendHwCheck').checked = hasHw;
...
// 2. دالة إغلاق الحصة وإرسال التقارير
async function confirmCloseSession(sendMessages) {
    ...
    if(!sendMessages) return showToast("تم إغلاق الحصة بدون إرسال رسائل.");
    ...
    try { await fetch('http://localhost:3000/status'); } 
    catch(e) { return showToast("السيرفر مغلق! برجاء تشغيل شاشة Node.js أولاً", "error"); }
    ...
    for (let st of groupStudents) {
        ...
        if(sendAtt) msg += `📋 *الحضور:* ${session.attendance[st.phone] === 'present' ? 'حاضر ✅' : 'غائب ❌'}\n`;
        if(sendExam && sessionExam) msg += `📝 *الامتحان:* ${sessionExam.grades[st.phone] ? sessionExam.grades[st.phone] + ' / ' + sessionExam.maxScore + ' ⭐' : 'لم يمتحن ⚠️'}\n`;
        if(sendHw && sessionHw) msg += `📚 *الواجب:* ${sessionHw.grades[st.phone] ? sessionHw.grades[st.phone] + ' / ' + sessionHw.maxScore + ' 📚' : 'لم يسلم ⚠️'}\n`;

        msg += `\nإدارة مستر ${TEACHER_NAME}`; 
...
async function syncDataToBot() {
    const dataToSync = {
        teacherName: TEACHER_NAME, // استخدام الاسم الثابت
    ...
    try { await fetch('http://localhost:3000/sync-database', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dataToSync) }); } catch (e) {}
}
// تشغيل المزامنة عند أي تغيير في البيانات (بما فيها الواجبات)
["students", "classSessions", "exams", "homeworks"].forEach(key => {
...
// تشغيل المزامنة عند أي تغيير في البيانات
["students", "classSessions", "exams"].forEach(key => {
    const originalSetItem = localStorage.setItem;
    localStorage.setItem = function(k, v) {
        originalSetItem.apply(this, arguments);
        if(key === k) syncDataToBot();
...
// تشغيل مزامنة أولية عند فتح الصفحة
setTimeout(syncDataToBot, 2000);
